...
#     created_user = fields.Many2one('res.users', string="Created User", readonly=True)
    ...
    created_user_id = fields.Many2one(comodel_name='res.users', default=lambda self: self.env.user, string='Created User')
    ...
    def date_validation(self):
        for rec in self:
            if rec.start_date and rec.end_date:
                ...
                if not start_date.month == end_date.month or not start_date.year == end_date.year:
